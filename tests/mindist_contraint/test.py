#!/usr/bin/python
''' This test checks the derivatives of the inequality 
   contraints for the minimal turbine distance constraint. 
'''
...
config.set_domain(domains.RectangularDomain(3000, 1000, 20, 3))
...
minconv = helpers.test_gradient_array(ieqcons_J, ieqcons_dJ, 
        numpy.array([1., 2., 3., 4., 7., 1., 6., 9.]))
if minconv < 1.99:
    info_red("Convergence test for the minimum distance constraints failed")
    sys.exit(1)
info_green("Test passed")    

ieq = generate_site_constraints(config, [[0, 0], [10, 0], [10, 10]], 
        penalty_factor=1)
# Only test the correctness of the first inequality constraint for simplicity
ieqcons_J = lambda m: ieq.function(m)[0]
ieqcons_dJ = lambda m, forget=False: ieq.jacobian(m)[0]
...
if not math.isnan(minconv):
    log(ERROR, "Convergence for the polygon shaped sites failed")
    sys.exit(1)
