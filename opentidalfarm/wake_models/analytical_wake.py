...
class AnalyticalWake(Expression):
    ...
    def flow_magnitude_at(self, turbines, index):

        def _tupleize(turbines):                                                
            """                                                                 
            Convenience function; turns a flattened array into
            tuples           
            """                                                                 
            return [(turbines[i*2], turbines[i*2+1]) for i in range(len(turbines)/2)]
        
        turbines = _tupleize(turbines)
        point = turbines[index]
        turbines_to_check = numpy.delete(turbines, index, axis=0)
        flow_velocity = self._flow_magnitude_at(point) 
        reduction_factor = self._combined_factor(point, turbines_to_check)/self.model.individual_factor(0,0) 
        return reduction_factor*flow_velocity 
